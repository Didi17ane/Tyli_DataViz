...
if not st.session_state.logged_in:
    ...
    df = pd.read_csv("./DATAS/ANSTAT2021_dataset_Clean.csv")
    df_ehcvm = df.copy()
    ...
    with st.sidebar:
        ...
        region = st.sidebar.selectbox("Région", ["Toutes"] + sorted(df_ehcvm["region_name"].unique().tolist()))
        age_grp = st.sidebar.selectbox("Tranche d’âge", ["Toutes"] + sorted(df_ehcvm["age_grp"].unique().tolist()))
        sex = st.sidebar.selectbox("sex", ["Tous"] + sorted(df_ehcvm["sex"].unique().tolist()))
        marital_status = st.sidebar.selectbox("Statut matrimonial", ["Tous"] + sorted(df_ehcvm["marital_status"].unique().tolist()))
    ...
    with col2:
        df_femme = data[data["sex"]=="Féminin"]
        femme = len(df_femme)
    ...
    with col3:
        df_homme = data[data["sex"]=="Masculin"]
        homme = len(df_homme)
    ...
    with tab1:
        ...
        with c1:
            ...
            st.divider()
            age_sex = data.groupby(["age_grp","sex"])["region_name"].count().unstack().fillna(0)
            age_sex.plot(kind="barh", stacked=True)
            st.pyplot(plt)
        with c2:
            # Zoom région: répartition des effectifs et indicateurs
            tab_region = data.groupby("region_name").agg(population=("region_name", "size"),
            ...
            st.divider()
            fig, ax = plt.subplots()
            tab_region.set_index("region_name")[["population", "revenu_median"]].plot(kind="bar", ax=ax, secondary_y="rev_median")
        ...
        age_grp = data.groupby("age_grp").agg(population=("region_name", "size"), revenu_median=("rev_total_mois","median")).reset_index()
    ...
    with tab2:
        ...
        with c_2:
            ...
            rev_grouped = data.groupby(["age_grp", "marital_status"]).agg(
    ...
    banc_moy=('bancarise', 'mean')
            ...
            bars = alt.Chart(rev_grouped).mark_bar().encode(
                ...
                tooltip=['age_grp', 'marital_status', alt.Tooltip('rev_median', format=',.2f')]
            )
